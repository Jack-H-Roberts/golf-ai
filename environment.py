...
class GolfEnv:
    ...
    def debug_print_hand(self, player_idx=0):
        """Prints a 3x3 grid of the first game's hand for a specific player."""
        from utils import TABLE_START, PLAYER_GRID_SIZE, SLOT_SIZE
        
        # Get the 135-bit segment for this player from the first game [game 0]
        start = TABLE_START + (player_idx * PLAYER_GRID_SIZE)
        hand_bits = self.state[0, start : start + PLAYER_GRID_SIZE]
        
        print(f"\n--- Player {player_idx} Hand (Game 0) ---")
        for row in range(3):
            row_str = ""
            for col in range(3):
                # Extract the 15 bits for this specific slot
                slot_start = (row * 3 + col) * SLOT_SIZE
                slot = hand_bits[slot_start : slot_start + SLOT_SIZE]
                
                # Logic: Check bit 14 (Unknown) or color bits
                is_unknown = slot[14] == 1.0
                color = "Red" if slot[0] == 1.0 else "Blue"
                
                if is_unknown:
                    row_str += f"[ ? ({color}) ] "
                else:
                    # Logic to find which face bit (1-13) is set
                    face_val = torch.argmax(slot[1:14]).item() + 1
                    row_str += f"[ {face_val} ({color}) ] "
            print(row_str)